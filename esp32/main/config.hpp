...
inline constexpr int         UART_BUF_SIZE   = 16'384;

// UART — Debug (only on chips with 3+ UARTs)
#if SOC_UART_NUM > 2
#define HAS_DEBUG_UART 1
...
inline constexpr int         WIFI_TX_POWER_DB = 80; // 0.25 dBm units: 80=20dBm (max), 32=8dBm
...
// FreeRTOS
inline constexpr uint32_t    TASK_STACK       = 12288;
inline constexpr UBaseType_t TASK_PRIO        = 5;
inline constexpr UBaseType_t QUEUE_SIZE       = 16;
